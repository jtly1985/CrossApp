...
//    pDirector->runWindow(RootWindow::create());
    ...
    sc->addRegisterCallback(register_all_autogentestbindings_js);
    
    sc->start();
    sc->enableDebugger();


//    bool ok =ScriptingCore::getInstance()->evalString("'hello '+'world, it is '+new Date()", NULL);
//    bool ok =ScriptingCore::getInstance()->evalString("cc.log(1111);", NULL);
